...
function ajax(params: AjaxParams) {
  ...
  if (params.method !== "GET") {
    switch (dataType) {
      ...
      default:
        // 如果是FormData，让浏览器自动设置Content-Type和boundary
 ...
 */
function getResultInfo(result: { code: number, msg: string, data: any }, responseType?: XMLHttpRequestResponseType) {
  const info: Api.Result = { code: result.code, msg: result.msg, data: result.data };
  switch (result.code) {
...
export default function request<T = any>(
  ...
  return new Promise<Api.Result<T>>(function (resolve) {
    ajax({
      ...
      success(res, xhr) {
        // getResultInfo数据处理
        // const info = getResultInfo({ code: xhr.status, data: res }, responseType);
        resolve(res);
      },
      fail(err) {
        const res = checkType(err.response) === "string" ? jsonParse(err.response) : err.response;
        // getResultInfo数据处理
        // const info = getResultInfo({ code: err.status, data: res });
        // 全局的请求错误提示可以写在这里
      ...
      onTimeout() {
        ...
        const info = getResultInfo({ code: config.requestTimeout, msg: "请求超时", data: {} });
        // 全局的请求超时提示可以写在这里
